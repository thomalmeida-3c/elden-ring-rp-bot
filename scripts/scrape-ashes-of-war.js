// Scraper único (offline) para popular data/game/ashes-of-war.json a partir do eip.gg.
// A listagem desta categoria só traz nome + imagem (sem afinidade/efeito na tabela).
const fs = require('fs');
const path = require('path');

const UA = 'Mozilla/5.0 (compatible; EldenRingRPBot-DataBuild/1.0)';
const OUT_PATH = path.join(__dirname, '..', 'data', 'game', 'ashes-of-war.json');
const DELAY_MS = 200;
const MAX_PAGES = 40;
const VIEW_COUNT = '99521-TCPID99518';
...
function extractListingRows(html) {
  ...
  while ((m = trRegex.exec(html))) {
    ...
    rows.push({
      id,
      name: name.replace(/^Ash of War:\s*/i, '').replace(/&#8217;/g, "'").trim(),
      link,
...
async function main() {
  console.log('Buscando listagem de ashes of war (paginação dinâmica)...');
  const listing = [];
  for (let page = 1; page <= MAX_PAGES; page++) {
    const url =
      page === 1
        ? 'https://eip.gg/elden-ring/db/ashes-of-war/'
        : `https://eip.gg/elden-ring/db/ashes-of-war/?wpv_view_count=${VIEW_COUNT}&wpv_paged=${page}`;
    const html = await fetchHtml(url);
    const rows = extractListingRows(html);
    console.log(`  página ${page}: ${rows.length} ashes of war`);
  ...
  console.log(`✅ Salvo em ${OUT_PATH} (${unique.length} ashes of war)`);
}
